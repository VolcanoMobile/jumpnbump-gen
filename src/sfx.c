...
void playJumpSfx()
{
    XGM_startPlayPCM(SFX_JUMP_1 + rnd(3), 1, nextPcmChannel++);
...
void playDeathSfx()
{
    XGM_startPlayPCM(SFX_DEATH_1 + rnd(3), 1, nextPcmChannel++);
    if (nextPcmChannel > SOUND_PCM_CH4) {
        nextPcmChannel = SOUND_PCM_CH1;
    }
}
...
void playSplashSfx()
{
    XGM_startPlayPCM(SFX_SPLASH_1 + rnd(3), 1, nextPcmChannel++);
    if (nextPcmChannel > SOUND_PCM_CH4) {
        nextPcmChannel = SOUND_PCM_CH1;
    }
}
...
void playSpringSfx()
{
    XGM_startPlayPCM(SFX_SPRING_1 + rnd(3), 1, nextPcmChannel++);
    if (nextPcmChannel > SOUND_PCM_CH4) {
        nextPcmChannel = SOUND_PCM_CH2;
    }
}
